...
public class TRMultiPara : Trajectory
{
	...
	Vector3 Tes(Transform target, Vector3 startPos, float speed)
	{
		...
		float sqrv22mv12 = Mathf.Sqrt(speed * speed - target.GetComponent<Rigidbody>().linearVelocity.sqrMagnitude);
		...
		if(Vector3.Angle(reverseVec, target.GetComponent<Rigidbody>().linearVelocity) <= 90f)
				...
				predictTime = (Mathf.Sqrt(reverseVec.sqrMagnitude + temp1 / sqrv22mv12 / sqrv22mv12) + Mathf.Sqrt(temp1 / sqrv22mv12 / sqrv22mv12)) / sqrv22mv12;
		Vector3 predictPos = target.position + target.GetComponent<Rigidbody>().linearVelocity * predictTime;
			return predictPos;
