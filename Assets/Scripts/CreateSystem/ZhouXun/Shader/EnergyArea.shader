...
//				float brightness_border = clamp(0.005-abs((_CenterAndRadius.w - dist)/_CenterAndRadius.w - 0.005),0,1)*50;
				...
				float4 retc = (0,0,0,0);
				retc.rgb = texc.rgb * ((_TintColor*(_BodyIntensity+exhaust_factor*_BodyIntensity*_ExhaustEffect) + _StreamColor*(xintensity+yintensity+zintensity+inc*inc*inc)*coef)*(brightness_body));
				retc.a = 1;
				return retc;
			}
			ENDCG
