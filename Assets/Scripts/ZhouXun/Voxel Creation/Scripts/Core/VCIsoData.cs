...
{
	// Version control
	public const int ISO_VERSION = 0x02030001;
	...
	public VCMaterial[] m_Materials = null;
	...
	public void DestroyMaterials()
	{
		if ( m_Materials != null )
		{
			if ( !m_Options.ForEditor )
			{
				foreach ( VCMaterial vcm in m_Materials )
				{
					if ( vcm != null )
					{
	...
	public bool IsPointIn ( IntVector3 pos )
	{
        ...
        if (m_HeadInfo.Category == EVCCategory.cgDbSword && pos.x > Mathf.CeilToInt(0.5f * m_HeadInfo.xSize - 6) && pos.x < Mathf.CeilToInt(0.5f * m_HeadInfo.xSize + 5)) return false;
    ...
    //public bool IsPointIn(IntVector3 pos,bool right)
    //{
    //    if (pos == null)
    //        return false;
    //    if (pos.x < 0) return false;
    //    if (pos.y < 0) return false;
    //    if (pos.z < 0) return false;
    //    if (pos.x >= 0.5f * m_HeadInfo.xSize) return false;
    //    if (pos.y >= m_HeadInfo.ySize) return false;
    //    if (pos.z >= 0.5f * m_HeadInfo.zSize) return false;
    //    return true;
    //}
    ...
    public IntVector3 ClampPointI(IntVector3 pos, bool right)
    {
        ...
        {
            int ivalue = 5;
            if (right && pos.x < 0) pos.x = 0;
            else if (!right && pos.x < Mathf.CeilToInt(0.5f * m_HeadInfo.xSize + ivalue)) pos.x = Mathf.CeilToInt(0.5f * m_HeadInfo.xSize + ivalue);
            else if (right && pos.x >= Mathf.CeilToInt(0.5f * m_HeadInfo.xSize - ivalue - 1)) pos.x = Mathf.CeilToInt(0.5f * m_HeadInfo.xSize - ivalue - 1);
            else if (!right && pos.x > m_HeadInfo.xSize) pos.x = m_HeadInfo.xSize - 1;
	...
	public byte[] Export()
	{
		using ( MemoryStream ms_iso = new MemoryStream () )
		{
            ...
            temp.LoadImage(m_HeadInfo.IconTex);
            m_HeadInfo.IconTex = temp.EncodeToJPG(25);
						...
						if ( guid != 0 )
						{
							...
							if ( m_Options.ForEditor )
							{
								...
								else
								{
                                    ...
                                    VCEAssetMgr.s_TempMaterials[guid] = vcmat;  //log:lz-2016.05.17 ֱ��Add�ᱨ����key�����Ѵ���
									m_Materials[i] = vcmat;
								}
							}
						...
						if ( guid != 0 )
						{
							...
							if ( m_Options.ForEditor )
							{
								...
								else
								{
                                    ...
                                    //VCEAssetMgr.s_TempDecals.Add(guid, vcdcl);    //log:lz-2016.05.17 ֱ��Add�ᱨ����key�����Ѵ���
